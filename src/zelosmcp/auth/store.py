"""Encrypted persistent token store for auth providers.

Mirrors the patterns established by :mod:`zelosmcp.savings_db`:
single aiosqlite connection, single asyncio.Lock for write
serialization, WAL journal mode, ``~/.zelosmcp/`` for the on-disk
path, ``":memory:"`` for tests.

Differences from the savings store:

- Token columns are encrypted at rest with Fernet (AES-128-CBC +
  HMAC-SHA256). The key lives at ``~/.zelosmcp/auth.key`` (chmod
  600), generated on first run if missing. In Kubernetes that key
  is mounted from a Secret rather than auto-generated; otherwise
  a Pod restart would lose every token.
- The ``device_sessions`` table tracks in-progress device flows so
  the SSE poll endpoint can survive a process restart mid-flow.
  Sessions older than their ``expires_at`` are skipped at read time
  and pruned lazily.
"""
