...
| `make nuke` | `clean` → remove `zelosmcp-*` volumes | Reset to factory state. Pincher re-indexes from scratch on next `make up`. |

### Service (interact with the running container)

| Verb | Chains | Intent |
|---|---|---|
| `make load` | POST `ZELOSMCP_CONFIG` → `index` (if `ZELOSMCP_WARM_ON_LOAD=1`) → `rule` (if `ZELOSMCP_RULE_ON_LOAD=1`) | Push backend config without restarting the container. Typical use: after editing `configs/default-zelosmcp.json`. |
...
| `make rule` | regenerate `ZELOSMCP_RULE_FILE` | Refresh the Cursor `.mdc`. Auto-chained from `load`. |
...
| `make ui` | `open http://localhost:$ZELOSMCP_PORT` | Opens the web UI in your browser. |
| `make kubeconfig` | add `zelosmcp` cluster + context to `$KUBERNETES_CONFIG_FILE` | Auto-called by `up`; exposed for manual re-add. |
| `make clean-kubeconfig` | remove `zelosmcp` cluster + context | Inverse of `kubeconfig`. |

### Development / debugging

| Verb | Chains | Intent |
|---|---|---|
| `make init-env` | run `scripts/init_env.py` | Interactive wizard, writes `.env` (and `configs/user-zelosmcp.json` when you opt out of any default backend). |
| `make setup` | `cert` → `setup-buildx` → `build` | One-shot first-time prep on a fresh machine. Slow (~3-5 min); idempotent. |
| `make build` | buildx build of `ZELOSMCP_IMAGE_TAG` | Incremental image build. |
...
| `make shell` | `docker exec -ti $ZELOSMCP_CONTAINER bash` | Bash inside the container. |
...
`make init-env` runs [`scripts/init_env.py`](../scripts/init_env.py): walks through the highest-impact knobs (USER_DATA_ROOT, port, bind address, Dockerfile choice, kubernetes/docker enable, Cursor rule scope/access mode), auto-detects `PLATFORM`, `DOCKER_SOCK_FILE`, and corp-cert presence, and writes a working `.env` end-to-end. When you opt out of any default backend, it also writes `configs/user-zelosmcp.json` (a subset of `configs/default-zelosmcp.json`) and points `ZELOSMCP_CONFIG` at it.
...
| `ZELOSMCP_PORT` | `8000` | Container-internal port zelosMCP listens on. Published to the host via `-p $ZELOSMCP_BIND_ADDR:$ZELOSMCP_PORT:$ZELOSMCP_PORT`. |
| `ZELOSMCP_BIND_ADDR` | `127.0.0.1` | Host address the published port binds to. `127.0.0.1` (default) means only this Mac can reach `:8000`; `0.0.0.0` exposes it on the LAN. |
| `ZELOSMCP_IMAGE_TAG` | `zelosmcp:dev` | Image tag the build produces. |
| `ZELOSMCP_CONTAINER` | `zelosmcp` | Container name `make up` creates. |
| `ZELOSMCP_CONFIG` | `configs/default-zelosmcp.json` | JSON config `make load` POSTs. |
| `ZELOSMCP_VOLUMES_FILE` | `configs/default-volumes.conf` | List of `docker run -v` specs. See [Volume mounts](#volume-mounts). |
| `ZELOSMCP_RULE_FILE` | `.cursor/rules/zelosmcp.mdc` | Where `make rule` writes the generated Cursor `.mdc`. Set to `$HOME/.cursor/rules/zelosmcp.mdc` for a global rule. |
| `ZELOSMCP_RULE_ACCESS` | `read-write` | Tool access mode in the rule body. Set to `read-only` to forbid `[mutates]`/`[destructive]` tools. |
| `ZELOSMCP_WARM_ON_LOAD` | `1` | Toggle the `load → index` chain. `0` for fast CI loads. |
| `ZELOSMCP_RULE_ON_LOAD` | `1` | Toggle the `load → rule` chain. `0` to keep an existing hand-edited `.mdc` in place. |
| `KUBERNETES_CONFIG_FILE` | `$HOME/.kube/config` | Host kubeconfig bind-mounted read-only. `make up` adds a `zelosmcp` cluster + context to it via `kubectl config set-cluster/set-context`. See [setup-rancher-desktop.md](setup-rancher-desktop.md). |
...
make load ZELOSMCP_CONFIG=configs/my-custom.json
make up ZELOSMCP_BIND_ADDR=0.0.0.0
```
...
## Volume mounts (`ZELOSMCP_VOLUMES_FILE`)

`make up` reads its `docker run -v` list from `$(ZELOSMCP_VOLUMES_FILE)` (default [`configs/default-volumes.conf`](../configs/default-volumes.conf)). One mount per line. Comments allowed (`#…` to end of line). Blank lines ignored. Shell variables (`$HOME`, `$USER_DATA_ROOT`, `$KUBERNETES_CONFIG_FILE`, `$DOCKER_SOCK_FILE`) are expanded at runtime.
...
- Named volumes: `zelosmcp-npm` (npx cache), `zelosmcp-cache` (uv/pip cache), `zelosmcp-pincher` (pincher SQLite DB), `zelosmcp-savings` (savings dashboard DB)
...
```bash
cp configs/default-volumes.conf ~/.config/zelosmcp-volumes.conf
# edit ~/.config/zelosmcp-volumes.conf — add lines like:
#   /Users/me/secrets/.aws:/root/.aws:ro
#   $HOME/.gitconfig:/root/.gitconfig:ro
#   zelosmcp-models:/opt/models
make up ZELOSMCP_VOLUMES_FILE=~/.config/zelosmcp-volumes.conf
```
